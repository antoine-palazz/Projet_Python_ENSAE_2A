...
def clean_df_usagers(df):
    ...
    df_usagers["sexe"].replace({1: "homme", 2: "femme", -1: np.nan}, inplace=True)

    df_usagers["catu"].replace(
        {1: "conducteur", 2: "passager", 3: "piéton", -1: np.nan}, inplace=True
    )
    ...
    df_usagers["etatp"].replace(
        {"-1": np.nan, 1: "seul", 2: "accompagné", 3: "en groupe"}, inplace=True
    )
...
def clean_df_caracs(df):
    ...
    df_caracs["agg"].replace({1: "hors agglo", 2: "agglo"}, inplace=True)
...
def clean_df_lieux(df):
    ...
    df_lieux["prof"].replace(
        {-1: np.nan, 1: "plat", 2: "pente", 3: "sommet de côte", 4: "bas de côte"},
        inplace=True,
    )
...
def clean_df_vehicules(df):
    ...
    df_vehicules["senc"].replace({-1: np.nan}, inplace=True)

    df_vehicules["catv"].replace({0: np.nan}, inplace=True)
    ...
    df_vehicules["obs"].replace({-1: np.nan}, inplace=True)
